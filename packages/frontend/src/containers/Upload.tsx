...
export default function Upload() {
  ...
  const [selectedChannelId, setSelectedChannelId] = useState<string>(
    userInfo!.settings.defaultChannelId,
  );
    ...
    try {
      ...
      if (session) {
        ...
        for (const [i, item] of session.items.entries()) {
          const imageBlob = await downloadFile(item.imageFile, (prog) =>
            setSessionLoadProgress(
              ((i + prog / 100) / session.items.length) * 100,
            ),
          );
  ...
  useEffect(() => {
    if (user) {
      ...
      const unsub = onSnapshot(
        doc(getFirestore(), "task_messages", user.uid),
        async (doc) => {
          const message = doc.data() as BaseMessage | undefined;
          if (!message || message.timestamp < Date.now() - 10 * 1000) {
            // 10 second timeout
            return;
          }
          ...
          switch (message.type) {
            case "error":
              ...
              displayError(
                t(errorMsg.message.i18nKey, errorMsg.message.data) as string,
              );
            ...
            case "success":
              ...
              displaySuccess(
                <Text>
                  <Trans
                    ...
                    values={{ duration: successMsg.elapsed }}
                    components={[
                      <Anchor href={successMsg.videoUrl} target="_blank" />,
                    ]}
                  />
                </Text>,
              );
              const i = uploadQueue.findIndex(
                (item) => item.id === successMsg.itemId,
              );
              if (i !== -1) {
                refreshUserInfo();
              ...
              setUploadingStatus(
                t("upload.upload-status", {
                  current: currentVideoUpload.current,
                  total: totalVideoUpload.current,
                }),
              );
  ...
  async function uploadVideos() {
    ...
    const request: UploadRequest = {
      ...
      videos: uploadQueue.map(({ imageFileBlob, ...rest }) => rest),
    };
    ...
    if (response !== undefined) {
      ...
      if (uploading < uploadQueue.length) {
        notifications.show({
          title: "Warning",
          message: t("upload.warning.quota-hit", {
            uploading,
  ...
  async function onFilesDropped(files: FileWithPath[]) {
    ...
    for (const [i, file] of files.entries()) {
      ...
      const queueSize = uploadQueue
        .map((item) => item.audioFileSize + item.imageFileSize)
        .reduce((t, a) => t + a, 0);
      const uploadItem = await getUploadItemFromFile(
        userInfo!,
        file,
        onProg,
        Math.max(0, quota - queueSize),
      );
      if (uploadItem) {
        ...
        >
          <Stack gap="0" w="100%">
            ...
            <Text c="dimmed" size="sm">
              <span style={{ maxWidth: "200px", display: "inline-flex" }}>
                <Text span truncate="end">
                  {item.originalAudioFileName}
                </Text>
              </span>{" "}
              | {formatDuration(item.audioFileLength)} |{" "}
              {formatBytes(item.audioFileSize)}
            </Text>
          </Stack>
          <UnstyledButton
